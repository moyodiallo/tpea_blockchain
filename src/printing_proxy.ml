...
let connect  ?(addr = Unix.inet_addr_of_string "::1" ) ?(port=12346) () =
  ...
  let%lwt listening_sock =  Server.create_listening_socket ~backlog:10
                              ~addr 12345 in
...
let send ch msg =
  Option.map
    (Messages.send ~verbose:false msg)
    ch
...
let receive ~name ch () =
  Option.map
    ((* Messages. *)receive ~verbose:false)
    ch |>
    function  None -> Log.log_error "No connection@";
            ...
            | Some t -> let%lwt t = t in
                        begin match
                          ...
                          Messages.message_of_yojson with
                        | Ok msg ->
                           Log.log_color
                             Log.orange
                             name
                        ...
                        | Error s ->
                           (Log.log_error "@[<v 2>Decoding error %s.@ \
                                           string: %s@ \
                                           yojson: %a@]@."
                              s
                              (Bytes.to_string t) 
                              (fun ppf t ->
                                Yojson.Safe.pretty_print ?std:None ppf
                                 (Yojson.Safe.from_string (Bytes.to_string t))
                              ) t;
                            Lwt.return_none)
                        end
let trace_send_error =
  function Some v -> v
...
let rec bounce_loop name input output =
  ...
  let%lwt _ =
    ...
    Some msg ->
     send output msg |> trace_send_error
