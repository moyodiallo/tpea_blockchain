...
let pk_of_yojson pkj =
  bigstring_of_yojson pkj >>?Hacl.Sign.unsafe_pk_of_bytes

let pk_to_bigstring pk=
  Hacl.Sign.unsafe_to_bytes pk
let pk_of_bigstring pkbs =
  Hacl.Sign.unsafe_pk_of_bytes pkbs


let pp_pk ppf pk  =
  pk_to_bigstring pk |> pp_bs ppf
let show_pk  pk  =
  pk_to_bigstring pk |> show_bs
...
let sk_of_yojson skj =
  bigstring_of_yojson skj >>?Hacl.Sign.unsafe_sk_of_bytes

let sk_to_bigstring sk=
  Hacl.Sign.unsafe_to_bytes sk
let sk_of_bigstring skbs =
  Hacl.Sign.unsafe_sk_of_bytes skbs

let pp_sk ppf sk  =
  sk_to_bigstring sk |> pp_bs ppf
let show_sk  sk  =
  sk_to_bigstring sk |> show_bs
